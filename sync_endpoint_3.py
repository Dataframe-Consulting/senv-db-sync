#!/usr/bin/env python3
"""
Sincronización del endpoint 3: vidrios_produccion
Total estimado: 88,826 registros
"""
...
from transforms_all_savio import transform_vidrios_produccion
import time
...
def main():
    """Sincroniza el endpoint vidrios_produccion con progreso visible"""
    
    print("=" * 80)
    print("🚀 SINCRONIZACIÓN CON PROGRESO VISIBLE")
    print("=" * 80)
    print("📊 Endpoint: vidrios_produccion")
    print("💾 Tabla: vidrios_produccion")
    print("🎯 Meta: 88,826 registros")
    print("=" * 80)
    print()
    ...
    oracle_config = OracleApexConfig(
        ...
        endpoint='vidrios_produccion'
    )
    
    supabase_config = SupabaseConfig(
        url=os.getenv('SUPABASE_URL'),
        key=os.getenv('SUPABASE_KEY'),
        table_name='vidrios_produccion'
    )
    ...
    try:
        ...
        while True:
            ...
            # Transformar
            transformed = [transform_vidrios_produccion(r) for r in records]
            ...
            if total - last_report_count >= report_interval or len(records) < sync_config.batch_size:
                ...
                remaining = 88826 - total
                eta_seconds = remaining / rate if rate > 0 else 0
                eta_minutes = eta_seconds / 60
                
                timestamp = datetime.now().strftime("%H:%M:%S")
                percentage = (total / 88826) * 100
                
                print(f"[{timestamp}] 📊 {total:,} / 88,826 ({percentage:.1f}%) | "
        ...
        print(f"📊 Progreso alcanzado: {total:,} / 88,826 registros")
        return 1
