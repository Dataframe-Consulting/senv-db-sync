#!/usr/bin/env python3
"""
Sincronización del endpoint 2: detalle_cotizacion
Total estimado: 16,055 registros
"""
...
from transforms_all_savio import transform_detalle_cotizacion
import time
...
def main():
    """Sincroniza el endpoint detalle_cotizacion con progreso visible"""
    
    print("=" * 80)
    print("🚀 SINCRONIZACIÓN CON PROGRESO VISIBLE")
    print("=" * 80)
    print("📊 Endpoint: detalle_cotizacion")
    print("💾 Tabla: detalle_cotizacion")
    print("🎯 Meta: 16,055 registros")
    print("=" * 80)
    print()
    ...
    oracle_config = OracleApexConfig(
        ...
        endpoint='detalle_cotizacion'
    )
    
    supabase_config = SupabaseConfig(
        url=os.getenv('SUPABASE_URL'),
        key=os.getenv('SUPABASE_KEY'),
        table_name='detalle_cotizacion'
    )
    ...
    try:
        ...
        while True:
            ...
            # Transformar
            transformed = [transform_detalle_cotizacion(r) for r in records]
            ...
            if total - last_report_count >= report_interval or len(records) < sync_config.batch_size:
                ...
                remaining = 16055 - total
                eta_seconds = remaining / rate if rate > 0 else 0
                eta_minutes = eta_seconds / 60
                
                timestamp = datetime.now().strftime("%H:%M:%S")
                percentage = (total / 16055) * 100
                
                print(f"[{timestamp}] 📊 {total:,} / 16,055 ({percentage:.1f}%) | "
        ...
        print(f"📊 Progreso alcanzado: {total:,} / 16,055 registros")
        return 1
